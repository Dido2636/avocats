...
class DataBaseV2():
    ...
    def insert_json_to_table(self, table_name:str, path:str) -> None:
        """
...
def scraping_acovat(n_page:str='1') -> dict:
    """
    Fonction qui permet de scraper la page des avocats.
    ...
    database = DataBaseV2(db_name='test_avocat', db_type='sqlite')
     # Tableau
    try:
        database.create_table('Tableau_avocat',
                        id_=db.Integer,
                        nom=db.String,
                        adresse=db.String,
                        telephone=db.String,
    ...
    response = requests.get(f"https://www.barreaudenice.com/annuaire/avocats/?fwp_paged={n_page}")
    ...
    try :
        with open('avocat.json', 'r') as json_file:
            avocat_dict = json.load(json_file)
    except:
        avocat_dict = {}


    for avocat in avocats:
        try: image = avocat.find('div', class_='avocat-img').img['src']
        except : image='NA'

        try: nom = avocat.find('h3').text.strip()
        except : nom='NA'

        try: adresse = avocat.find('span', class_='adresse').text.strip()
        except : adresse='NA'

        try: telephone = avocat.find('span', class_='telephone').text.strip()
        except : telephone='NA'

        try: email = avocat.find('span', class_='email').a.text.strip()
        except : email='NA'

        try: id = avocat.find('div', class_ = 'avocat-infos').span.text.strip().split(' : ')[1]
        except : id='NA'
        ...
        database.insert_row('Tableau_avocat',id_=int(id), 
                            nom=nom, 
                            adresse=adresse, 
                            telephone=telephone, 
                            email=email)
    ...
    with open(f'avocat.json', 'w') as json_file: json.dump(avocat_dict, json_file, ensure_ascii=False, indent=4)
